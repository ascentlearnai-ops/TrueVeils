...
const { computeSessionVerdict } = require('../review/verdict');
...
async function generate(data) {
  ...
  const verdict = data.verdict || computeSessionVerdict({ scores, review, behavior: risk.behavior });
  const avg = risk.avg;
  const max = risk.max;
  const highFlags = flags.filter(f => f.severity === 'high' || f.severity === 'critical').length;

  const html = buildHtml({
    session, startMs, endMs, duration, avg, max, highFlags, risk, review, verdict, notes,
...
function buildHtml(ctx) {
  const { session, startMs, endMs, duration, risk, review, verdict = null, notes = [], transcripts, flags, audioChunks, totalResponses } = ctx;
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .verdict-panel { margin-bottom: 8px; }
  .verdict-row { display: flex; justify-content: space-between; align-items: flex-start; gap: 24px; flex-wrap: wrap; }
  .verdict-label { font-size: 26px; font-weight: 800; letter-spacing: -.6px; }
  .verdict-label.high { color: #f87171; }
  .verdict-label.med { color: #fbbf24; }
  .verdict-label.low { color: #4ade80; }
  .verdict-sub { margin-top: 6px; font-size: 13px; color: rgba(255,255,255,.5); }
  .verdict-basis { font-size: 12px; color: rgba(255,255,255,.55); line-height: 1.9; text-align: right; }
  .verdict-basis b { color: rgba(255,255,255,.85); font-weight: 600; }
...
<body>
  <div class="container">
    ...
    <div class="panel verdict-panel">
      <div class="verdict-row">
        <div class="verdict-main">
          <div class="verdict-label ${verdict.verdictScore != null && verdict.verdictScore >= 70 ? 'high' : verdict.verdictLabel === 'Likely unassisted' ? 'low' : 'med'}">${esc(verdict.verdictLabel)}</div>
          <div class="verdict-sub">Confidence: ${esc(verdict.verdictConfidence)}${verdict.verdictScore != null ? ` · Estimate ${esc(verdict.verdictScore)}/100` : ''}</div>
        </div>
        <div class="verdict-basis">
          <div>Scored response windows: <b>${esc(verdict.basis.transcriptWindows)}</b></div>
          <div>Transcript-pattern average: <b>${esc(verdict.basis.transcriptAverage)}</b></div>
          <div>Behavioral review band: <b>${esc(review.displayBand)}</b></div>
          ${verdict.basis.behavioralDestinations.length ? `<div>Restricted destinations: <b>${esc(verdict.basis.behavioralDestinations.join(', '))}</b></div>` : ''}
        </div>
      </div>
      <div class="reasoning">${esc(verdict.advisoryNote)}</div>
    </div>` : ''}
    ...
    <footer>
      ...
      This report is advisory evidence for human review. It is not an automated hiring decision.<br>
      Transcript-pattern weights are expert-configured and not statistically calibrated on a large labeled dataset.
    </footer>
  </div>
</body>
</html>`;
